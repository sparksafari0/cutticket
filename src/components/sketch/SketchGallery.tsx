...
export const SketchGallery = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sketches.map(sketch => <Card key={sketch.id} className="cursor-pointer hover:shadow-lg transition-shadow relative" onClick={() => onSketchClick?.(sketch)}>
            <Button variant="ghost" size="icon" onClick={e => handleDeleteClick(sketch.id, e)} className="absolute top-2 right-2 z-10 text-red-500 hover:text-red-700 bg-red-100">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                {sketch.visualized_image && <div className="space-y-1">
                    ...
                    <div className="relative aspect-square bg-gray-100 rounded-md overflow-hidden group">
                      <img src={sketch.visualized_image} alt="Visualized" className="w-full h-full object-contain cursor-pointer" onClick={e => handleImageClick(sketch.visualized_image!, 'Visualized Image', e)} />
                      <Button size="icon" variant="secondary" className="absolute bottom-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6" onClick={e => downloadImage(sketch.visualized_image!, 'visualized-image.png', e)}>
                        <Download className="h-3 w-3" />
                      </Button>
                    </div>
                  </div>}
                {sketch.flat_sketch_image && <div className="space-y-1">
                    <div className="text-xs text-gray-600">Flat Sketch</div>
                    <div className="relative aspect-square bg-gray-100 rounded-md overflow-hidden group">
                      <img src={sketch.flat_sketch_image} alt="Flat Sketch" className="w-full h-full object-contain cursor-pointer" onClick={e => handleImageClick(sketch.flat_sketch_image!, 'Flat Sketch', e)} />
                      <Button size="icon" variant="secondary" className="absolute bottom-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6" onClick={e => downloadImage(sketch.flat_sketch_image!, 'flat-sketch.png', e)}>
                        <Download className="h-3 w-3" />
                      </Button>
                    </div>
                  </div>}
