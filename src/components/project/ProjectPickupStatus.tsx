...
const ProjectPickupStatus = ({
  project,
  onPickedUpChange
}: ProjectPickupStatusProps) => {
  if (project.status !== 'completed') return null;
  return <div className="space-y-3">
      
      {onPickedUpChange ? <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={() => onPickedUpChange(true)} className={`w-full justify-start ${project.pickedUp === true ? 'bg-green-100 border-green-500 ring-1 ring-green-500' : ''}`}>
          ...
          <Button variant="outline" onClick={() => onPickedUpChange(false)} className={`w-full justify-start ${project.pickedUp === false ? 'bg-red-100 border-red-500 ring-1 ring-red-500' : ''}`}>
          ...
          {project.pickedUp ? <span className="flex items-center text-green-600">
